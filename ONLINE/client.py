import pygame as p
from ONLINE.chess_board_client import board_game,Move
...
def start_online(win,ip,arr_options):
    global COLOR_SQ_1,COLOR_SQ_2,pm_color,COLOR_HIGH_1,alpha,join,screen
    screen = win
...
''' create thread '''
def create_thread(target): # a thread to block the main thread
    thread = threading.Thread(target=target)
    thread.daemon = True
    thread.start()


''' receive data = change turn, if player is quited, board to be updated '''
def receive_data():
    global TURN,sock
    while 1:
...
os.environ['SDL_VIDEO_WINDOW_POS'] = '1100,100'
...
place_names = ["br", "bn", "bb", "bq", "bk", "bb", "bn", "br", "bp", "--", "wr", "wn", "wb", "wq", "wk", "wb", "wn",
               "wr", "wp"]  # all possible places on board

''' unpack from board str to normal board and copy it to game state board '''
...
#----------------------------------------------------------------------------------------------------------------------------------
        ...
        while running:
            ...
            # if not turn, wait for the other player to finish
